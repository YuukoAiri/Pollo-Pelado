...
export function SidebarNav({ className }: { className?: string }) {
  ...
  const mainLinks = NAV_LINKS.filter(link => !['/settings', '/analytics'].includes(link.href));
  const systemLinks = NAV_LINKS.filter(link => ['/settings', '/analytics'].includes(link.href));
                 ...
                 <div className="flex items-center justify-between p-2 rounded-lg hover:bg-sidebar-accent cursor-pointer">
                    <div className="flex items-center gap-2">
                        <Avatar className="h-9 w-9 rounded-md">
                            <AvatarImage src={user?.photoURL || undefined} alt={user?.displayName || "Admin"} data-ai-hint="person" />
