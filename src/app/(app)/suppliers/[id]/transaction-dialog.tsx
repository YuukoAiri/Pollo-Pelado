...
export function TransactionDialog({ isOpen, setIsOpen, supplierId, supplierName, payment }: TransactionDialogProps) {
  const { user } = useUser();
  ...
  const handleSubmit = async (data: PaymentFormData) => {
    ...
    try {
        if (payment) { // Editing
            const docRef = doc(firestore, 'users', user.uid, 'supplier_payments', payment.id!);
  ...
  const title = payment ? `Editar Pago a ${supplierName}` : `Registrar Pago a ${supplierName}`;
  const description = payment ? "Modifica los detalles de este pago." : "Añade un nuevo pago realizado a este proveedor.";
                ...
                <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4 py-4">
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            ...
                            <Select onValueChange={field.onChange} value={field.value}>
                                ...
                                <SelectContent>
                                {PAYMENT_METHODS.filter(m => m.value !== 'Crédito').map((method) => (
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            ...
                            <FormControl><Textarea placeholder="Ej: Pago parcial de factura #123" {...field} /></FormControl>
                            <FormMessage />
                            </FormItem>
                        )}
                    />
